...
PROBE(){
  ...
  # Terminate probe process every night at 11:55 PM
  if [[ "${GLOBLE_TIME}" > "23:55:00" ]] && [[ "${GLOBLE_TIME}" < "23:55:50" ]] && [[ "${COUNTER}" -ge 1 ]]; then
    kill -15 $(pgrep -f '[c]lient-linux.py')
    echo "${LOG_TIME} [INFO] Probe process terminated." >> ./main.log
  fi
}
